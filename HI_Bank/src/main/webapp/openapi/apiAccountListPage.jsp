...
<h1>조회</h1>
<%
    out.println(name + " 고객님의 타행 은행 계좌");
%>
<br/>
<%
...
<script>
    ...
    function fetchData() {
        ...
        $.ajax({
            ...
            method: "GET",
            data: { identityNum: "111111-1111111" },
            dataType: "json",
            success: function(data, status) {
                ...
                data.forEach(function(account) {
                    ...
                    var formTd = $("<td>");
                    var form1 = $("<form>").attr("action", "accountManage.bank").attr("method", "post").appendTo(formTd);
                    var form2 = $("<form>").attr("action", "apiAccountTransferPage.bank").attr("method", "post").appendTo(formTd);
                    ...
                    $("<input>").attr("type", "submit").attr("value", "계좌관리").appendTo(form1);
                    ...
                    $("<input>").attr("type", "submit").attr("value", "이체").appendTo(form2);
...
<h3>TH은행</h3>
<table id="TH-table" >
    <tr style="background-color:#666; color:#fff; border: none; padding:10px; height:35px;">
        <th>은행코드</th>
        <th>상품분류</th>
        <th>계좌명</th>
        <th>계좌번호</th>
...
<table id="ALMOND-table" >
    <tr style="background-color:#666; color:#fff; border: solid 1px #666; padding:10px; height:35px;">
        <th>은행코드</th>
        <th>상품분류</th>
        <th>계좌명</th>
        <th>계좌번호</th>
